...
function showFirstAndLast(arr) {
  ...
  arr.forEach(function (word) {
    newArr.push(word.charAt(0) + word.charAt(word.length - 1));
  });
  return newArr;
  //OR
  //   arr.forEach(function(word) {
...
function doubleOddNumbers(arr) {
  return arr
    .filter(function (value) {
      return value % 2 !== 0; //this filters out all the numbers that are not even (odd)
    })
    .map(function (value) {
      return value * 2; //this takes the filtered numbers, then multiples them by 2
    });
}
